...
// **আপনার ব্যক্তিগত Firebase কনফিগারেশন এখানে থাকতে হবে**
...
function saveToLocalStorage() {
    ...
    updateTotalCost();
    renderAllLists(); // সেভ করার পর রেন্ডার নিশ্চিত করা
}
...
function uploadDokanImage(listId, file, callback) {
    if (!userId) {
        alert("লগইন না করা থাকলে ছবি আপলোড করা যাবে না।");
        return;
    }
...
exportBtn.addEventListener('click', () => {
    ...
    alert("ডেটা ফাইল ডাউনলোড হয়েছে!");
});
        ...
        reader.onload = (event) => {
            try {
                ...
                if (importedData.bazarLists && importedData.dokanBakiiLists && importedData.billPaymentsLists && importedData.trash && importedData.archivedLists) {
                    ...
                    // ইম্পোর্ট করার পর স্থানীয় এবং Firebase উভয় জায়গায় ডেটা সেভ করা
                    saveToLocalStorage();
                    if (userId) {
                        saveToFirebase('lists', bazarLists);
                        saveToFirebase('dokanBakii', dokanBakiiLists);
...
function initializeApp() {
    ...
    auth.onAuthStateChanged((user) => {
        if (user) {
            ...
            userId = null;
            console.log('Authentication state changed: User is logged out. Loading data from Local Storage.');
            updateUI(null);
            
            // লগআউট অবস্থায় লোকাল স্টোরেজ থেকে ডেটা লোড করা
            bazarLists = JSON.parse(localStorage.getItem('bazarLists') || '[]');
            dokanBakiiLists = JSON.parse(localStorage.getItem('dokanBakiiLists') || '[]');
            billPaymentsLists = JSON.parse(localStorage.getItem('billPaymentsLists') || '[]');
            trash = JSON.parse(localStorage.getItem('bazarTrash') || '[]');
            archivedLists = JSON.parse(localStorage.getItem('bazarArchived') || '[]');
...
function loadAllData() {
    ...
    database.ref('users/' + userId + '/lists').on('value', (snapshot) => {
        ...
        saveToLocalStorage(); // Firebase থেকে লোড করার পর Local Storage-এও সেভ করা
    });
